 ...
 *
 *  - text over a surface axe declines to resolve. `.cockpit` — the box around
 *    the whole attack, its sixteen counters, its verdict and every scenario
 *    button — paints `linear-gradient(180deg, var(--accent-soft) 0, transparent
 *    12rem)` over `--panel`, so the surface under everything in the first
 *    screen of this page is a gradient that fades out partway down the box.
 *    axe files that under `incomplete`. It is also the case a naive gradient
 *    reader gets wrong: judging the gradient at its worst stop would measure
 *    the tinted top against text that sits below the fade, so this walk samples
 *    each layer AT THE TEXT'S OWN POSITION instead.
 *  - text faded by an ancestor's `opacity` — axe reads the declared `color`,
 *    which is not the colour that lands on screen. Here the only element that
 *    does it is `.seg-input` (`opacity: 0`), the visually-hidden radio behind
 *    every segmented control; that one owns no text, and the walk skips it.
 *    The guard stays because it is what makes that a measurement.
 *
 ...
 *    eleven `overflow-x: auto` boxes and they hold most of its evidence:
 *    `.ranking-wrap` (the sixteen counters), `.lat-grid-wrap` (a 16x16 table),
 *    `.trace-wrap` (the round-by-round encryption walkthrough), `.compare-wrap`
 *    (a seven-row comparison), `.decay-wrap`, `.lat-evidence-wrap`, and every
 *    `.equation` and `.formula`. Content scrolled past those is not dimmed or
 ...
 *    figure this lab draws — the LAT grid, the trail steps, the decay bars, the
 *    success-rate curve — is HTML and CSS, and the page's only SVG is the
 *    shared header's two glyph-free `aria-hidden` marks. The branch is kept
 *    because the alternative is a helper that silently mismeasures the first
 *    labelled SVG figure anyone adds: an ancestor-only walk would composite a
 ...
 * uses. The one genuinely short-lived state here is `verdict is-counting`,
 * which `main.ts` paints for roughly 900ms of `requestAnimationFrame` while the
 * sixteen counters fill — and it is unreachable under the reduced motion this
 * gate asserts, because `runAttackAnimated` branches on `prefersReducedMotion`
 * and advances the counter to completion in one call. That is stated in
 * `gate.ts` as a deliberate, named coverage gap rather than papered over.
 *
 * The parameter is kept because the narrowed form is what makes a short-lived
 * state measurable at all: a class that lives 900ms is shorter than a full axe
...
export async function auditContrast(page: Page, within = 'body *'): Promise<ContrastFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * page: `.cockpit` paints `linear-gradient(180deg, var(--accent-soft) 0,
     * transparent 12rem)` over a box several screens tall, so its tint is real
     * for the heading and gone by the time the counters are reached. Judging it
     * at its worst stop would report the tinted surface for text that sits well
     * below the fade. So each gradient is *sampled* at the text's real location
     ...
     * unexercised on this page and kept for the same reason as the SVG branch
     * above — the day a wash appears, an unsampled gradient is a wrong number
     ...
     *
     * A driven pass here walks eight sections and — the expensive part — the
     * 16x16 linear approximation table, which is 256 `<button>` cells, beside a
     * sixteen-row counter ranking that renders a bar and three numbers per row.
     * That is close to a thousand sibling elements all re-walking the same
     ...
     * that is `.ranking-wrap`, `.lat-grid-wrap`, `.trace-wrap`, `.compare-wrap`,
     * `.decay-wrap`, `.lat-evidence-wrap`, `.equation` and `.formula`, all
    ...
    const svgUnderlay = (el: Element, box: DOMRect): RGBA => {
      ...
      // itself — the LAT grid, the trail, the decay bars, the success curve —
      // is HTML and CSS.
    ...
    const isVisible = (el: Element): boolean => {
      ...
      // the five disclosures this page ships shut: `.tune` ("Change the
      // target", which is the only route to the S-box, target-nibble,
      // approximation-mode and custom-mask controls), `.jargon`, and three
      // `.deep` panels. The gate opens each by clicking its <summary>, which is
      // the route a reader has, rather than setting `.open` from script —
      // which is what the gate this replaces did to all of them at once, right
      ...
      // carries the `<caption>` of all three data tables — the sixteen-candidate
      // ranking, the 16x16 LAT, and the differential-versus-linear comparison.
      // Each has a rect and passes `checkVisibility()` while painting exactly
     ...
     *
     * What this lab hides was enumerated rather than assumed. Inside `#app` it
     * is the `.verdict-icon` glyph (⚠ ≈ ✓ ○ ↻ ! ◐) that opens every verdict
     * head, and the shared header's two decorative `<svg>` marks. The verdict
     * glyph is the redundant half of a redundant pair: the WORD beside it
     * ("KEY RECOVERED", "NOT RECOVERED", "PARTIAL", "CONFIGURATION CHANGED",
     * "APPROXIMATION REJECTED") sits in the same ink on the same surface in the
     * same `.verdict-head`, so measuring the head measures the glyph too.
     *
     * Nothing on this page hides a VALUE behind `aria-hidden`. No counter, no
     * bias, no LAT cell and no verdict headline is inside such a subtree —
     ...
     *
     * This lab has no SVG of its own. The guard stays because the usual way the
     * mistake arrives is exactly how this lab builds its DOM: `innerHTML`
     * template literals, used for every counter row, every LAT cell, every
     * trail step and every segmented control. Interpolating a string ARRAY into
     ...
     * page runs eight sections deep, so the day the declaration moves from
     * `html` to `body` — a change nobody would think of as risky — every
     * element below the first screen would otherwise report against a white
     * page that does not exist, in a theme whose canvas is `#0c0e1a`.
     */
