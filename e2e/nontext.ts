...
export async function auditNonText(page: Page, within = 'body *'): Promise<NonTextFailure[]> {
  return page.evaluate((rootSelector) => {
     ...
     * transparent 12rem)`, and the controls it contains straddle the fade — the
     * Run button sits in the tint, the scenario buttons well below it. So each
     * gradient is *sampled* at the element's real location instead: linear by
    ...
    const CONTROL = [
      ...
      // covers that shape too. It is live inside `#app` here: the hero's call
      // to action is `<a class="btn btn-primary btn-lg" href="#attack">`, which
      // is drawn exactly like the Run button beside it and would be judged by
      // nothing without this entry. Restricted to link-shaped BUTTONS — prose
      // links identify themselves by their text and are not 1.4.11 cases.
      'a[role=button]',
      'a[class*=btn]',
      'a[class*=button]',
    ].join(',');
